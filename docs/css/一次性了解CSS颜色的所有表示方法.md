---
title: 一次性了解 CSS 颜色的所有表示方法
meta:
  - name: description
    content: CSS 颜色的表示
  - name: keywords
    content: CSS,Color
banner: /20200211/daimei.gif
created: 2020-02-11
updated: 2020-02-22
tags:
  - CSS
...
这篇讲的是 CSS3 的颜色属性值的表示方法。

Github 地址:

<https://github.com/UESTCzhouyuchuan/CSS/tree/master/color>
...
currentColor 代表当前元素设置的`color`值,或者最近的祖先元素设置的`color`值。
例如:
...
前两个值为`R`值，中间两个值为`G`值，后两个值`为`B 值。即对应`RGB`
...
**缩写的格式**: `#663399` === `#639`，即 6 个 16 进制简写成 3 个，实际值是每个值的双拼写的组合。

### 8 位表示法。

多出来的最后两位表示`A`透明度。对应`RGBA`

## RGBA

A 是 alpha 即透明度，值在 0-1，0 为完全透明，1 不透明。
...
![hue.png](../.vuepress/public/20200211/hue.png)

### S

S 为 saturation,饱和度。表示色彩鲜艳程度,0%-100%,饱和度越高，颜色越鲜艳。

![saturation.png](../.vuepress/public/20200211/saturation.png)

### L

L 为 lightness,亮度。表示色彩明暗程度,0%-100%,值越高，颜色越亮。

![lightness.png](../.vuepress/public/20200211/lightness.png)

## HSLA

和 RGBA 类似，A 是 alpha 即透明度，值在 0-1，0 为完全透明，1 不透明。
...
<table id="colors_table">
 ...
 <tbody>
  ...
  <tr>
   ...
   <td style="text-align: center;"><code>darkslategray</code><sup><a href="#1" title="color_value#1">[*]</a></sup>（墨绿）</td>
  ...
  <tr>
   ...
   <td style="text-align: center;"><code>floralwhite</code>（花白）</td>
  ...
  <tr>
   ...
   <td style="text-align: center;"><code>mediumturquoise</code>（中海湖蓝）</td>
